...
\begin{algorithmic}[1]
    \Function{GSF}{$G = (V, E), \{T_1, T_2, \dots, T_t\}, w$}
        ...
        \State $\mathcal{T} \leftarrow \bigcup_{i=1}^t T_i$
...
\begin{proof}
We apply the local ratio technique.

\textbf{Step 1: Weight Decomposition.}
The algorithm decomposes the weight function as:
\[
...
\begin{lemma}
    For any $j$, $w^{(k)}(e) \ge 0$ for all $e \in E$.

    \begin{proof}
        By induction on the number of iterations.
        \begin{itemize}
            \item Base case: $k = 0$. $w^{(0)}(e) = w(e) \ge 0$
            \item Inductive step: assume $w^{(k)}(e) \ge 0$ for all $e \in E$, then since $\epsilon_j \cdot w'_j(e) \le w(e)$, we have $w^{(k+1)}(e) = w(e) - \epsilon_j \cdot w'_j(e) \ge 0$
        \end{itemize}
    \end{proof}
\end{lemma}

\textbf{Step 2: Cost of the Solution.}
Edges are added to $F$ only when they become tight (weight reaches 0). Since weights only decrease, every edge in $F$ satisfies $w^{(k)}(e) = 0$. Pruning only removes edges, so:
\[
    w(F) = \sum_{j=1}^k \epsilon_j \cdot w'_j(F)
\]

\textbf{Step 3: Lower Bound on OPT.}
For any feasible solution $X$, and $X = OPT$ in particular:
\[
    w(X) = \sum_{j=1}^k \epsilon_j \cdot w'_j(X) + w^{(k)}(X) \geq \sum_{j=1}^k \epsilon_j \cdot w'_j(X)
\]

\textbf{Step 4: Applying Part (a).}
We show that for each iteration $j$: $w'_j(F) \leq 2 \cdot w'_j(OPT)$.

In each iteration, $w'_j(u,v) = |\{u,v\} \cap \mathcal{T}_j|$ where $\mathcal{T}_j \supseteq T = \bigcup_i T_i$ is the current active set. Since the pruning step ensures $F$ is a \emph{minimal} GSF, we apply part (a): the returned forest $F$ is a 2-approximation with respect to $w'_j$.
...
\begin{itemize}
    \item Lower bound: Each original terminal $v \in T$ must have $\deg_{OPT}(v) \geq 1$, so $w'_j(OPT) \geq |T|$.
    \item Upper bound: After pruning, $F$ is a forest with no non-terminal leaves, so $w'_j(F) \leq 2|T|$ by the degree-sum argument in part (a).
\end{itemize}
