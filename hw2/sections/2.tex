...
\begin{lemma}
The set $H$ returned by the algorithm is a valid hitting set for $\mathcal{C}$.
\end{lemma}

\begin{proof}
We proceed by induction on the number of elements with strictly positive weight, denoted by $k = |\{u \in U \mid w(u) > 0\}|$.

\paragraph{Base Case ($k=0$):}
If there are no elements with positive weight, then $w(u) = 0$ for all $u$, hence $U_0 = U$. In particular, $U_0$ hits all sets (each $S_i$ is non-empty and $S_i \subseteq U$), so the algorithm terminates at Step 2 and returns a valid hitting set.

\paragraph{Inductive Step:}
Let $k > 0$. Assume the algorithm returns a valid hitting set for any instance with fewer than $k$ positive-weight elements.
Consider the current instance with $k$ positive-weight elements. The algorithm constructs a weight function $w_2$ where at least one element $u^* \in S$ (which previously had $w(u^*) > 0$) now has $w_2(u^*) = 0$.
Thus, the recursive call using $w_2$ has strictly fewer than $k$ positive-weight elements.
By the inductive hypothesis, this recursive call returns a set $H$ that is a valid hitting set for $\mathcal{C}$.
Since the current step simply returns this same set $H$, the output is a valid hitting set for the current instance.
\end{proof}
