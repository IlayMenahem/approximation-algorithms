...
\usepackage{enumitem} % For custom labels in enumerate
% Define the custom commands used in your code
\newcommand{\brs}[1]{\left[ #1 \right]}   % Brackets []
\newcommand{\brc}[1]{\left\{ #1 \right\}}   % Braces {}
\newcommand{\br}[1]{\left( #1 \right)}    % Parentheses ()

% Define Lemma environment if not already present
