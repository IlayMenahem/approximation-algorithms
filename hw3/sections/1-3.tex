...
We will prove by induction on $\sum_{n_i \in n}{n_i}$.
...
State Space Size: The DP table has one entry for every possible vector $\mathbf{n}$. Since each $n_i$ can range from $0$ to $n$ (the total number of jobs), there are $O(n^d)$ possible states.\\Transition Cost: For each state, we iterate over the set $C$, which represents all valid machine configurations.
...
\end{align*}
Subtracting $p_m C_m$ from both sides of the violation inequality (1)
\begin{align*}
...
&= \sum_{j \in A \setminus {m}} p_i p_j + \frac{p_i p_m}{2} \
&< \sum_{j \in A \setminus {m}} p_i p_j + (p_i p_m) + \frac{p_i^2}{2} \tag{Since $p_i, p_m, p_i^2 > 0$} \
&= \sum_{j \in A} p_i p_j + \frac{p_i^2}{2}
\end{align*}
Adding this to the violation inequality (1)
\begin{align*}
